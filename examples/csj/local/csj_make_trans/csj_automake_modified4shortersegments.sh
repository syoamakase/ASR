...
# Copyright  2015 Tokyo Institute of Technology (Authors: Takafumi Moriyaand Takahiro Shinozaki)
...
k2phone=./local/csj_make_trans/kana2phone_rule.ipa.staka-utf8
vocab2dic=./local/csj_make_trans/vocab2htkdic_staka_utf8.pl
...
cp ./local/csj_make_trans/generate_dir.txt $outd/README.txt
...
for vol in dvd{3..17} ;do
	...
	rm -r $outd/$vol/00README.txt
        ...
        # $csjconnect 0.3 8 $outd/$vol/$id/${id}.4trn.t $id > $outd/$vol/$id/${id}-trans.text
...
## Exclude evaluation data and duplication data from dvd*
if [ ! -d ./csj-data/[eval,dup_other] ]; then
(
    mkdir -p $outd/eval
    mkdir -p $outd/dup_other
    mkdir -p $outd/eval/eval1
    mkdir -p $outd/eval/eval2
    mkdir -p $outd/eval/eval3
    
    # Same speaker including evaluation data (eval2 : A01M0056)
    rm dup_list
	...
	mv $list $outd/dup_other
	cp dup_list $outd/dup_other/duplication.list
    done
    wait
